...
const ui = {
  ...
  buildDateText: document.getElementById("build-date-text"),
  copyDebugBtn: document.getElementById("copy-debug-btn"),
  clearAppDataBtn: document.getElementById("clear-app-data-btn"),
};
...
function init() {
  ...
  ui.appVersionText.textContent = `v${APP_VERSION}`;
  ui.buildDateText.textContent = `Build date: ${BUILD_DATE}`;
  setScreen("home");
  renderAll();
}
...
function bindEvents() {
  ...
  ui.copyDebugBtn.addEventListener("click", copyDebugInfo);
  ui.clearAppDataBtn.addEventListener("click", clearAppData);
...
function copyDebugInfo() {
  ...
  ui.updateStatus.textContent = "Debug info copied to clipboard.";
}

function clearAppData() {
  const ok = window.confirm("Reset all local app data and reload? This cannot be undone.");
  if (!ok) return;
  ["something-to-focus-v1","something-to-focus-v2","something-to-focus-v3","something-to-focus-v4","something-to-focus-v5","something-to-focus-v6","something-to-focus-v7"].forEach((k)=>localStorage.removeItem(k));
  location.reload();
}
